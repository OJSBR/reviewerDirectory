			...
			<form class="rd-nom-form" method="get" action="{$directoryUrl|escape}">
				...
				<div class="rd-field">
					...
					<select id="rdIssueId" name="rdIssueId">
						<option value="0">{translate key="plugins.generic.reviewerDirectory.allIssues"}</option>
						{foreach from=$issues item=iss}
							<option value="{$iss.id}"{if $iss.id == $rdIssueId} selected{/if}>{$iss.label|escape}</option>
						{/foreach}
					</select>
				</div>
				<div class="rd-field">
