...
class ReviewerDirectoryHandler extends Handler
{
    ...
    public function index($args, $request)
    {
        ...
        $nominata = $nominataRequested
            ? $this->getNominata($contextId, $dateFrom, $dateTo, $issueId)
            : null;
    ...
    /**
     * Lista de edições da revista para o seletor da nominata.
     *
    ...
    /**
     * Nominata: avaliadores que CONCLUÍRAM avaliações no período e/ou edição.
     *
    ...
    protected function getNominata(int $contextId, string $dateFrom, string $dateTo, int $issueId): array
    {
        ...
        if ($issueId > 0) {
            $submissionIds = DB::table('publications')
                ...
                ->pluck('submission_id')
                ->all();
            // Sem submissões na edição: nominata vazia.
            $query->whereIn('ra.submission_id', empty($submissionIds) ? [0] : $submissionIds);

            $issue = Repo::issue()->get($issueId);
            $issueLabel = $issue ? $issue->getIssueIdentification() : '';
        }
